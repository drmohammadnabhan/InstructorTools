# Full code block with integrated fixes for issues 1-8
...
# FIX 3: Modify calculate_stats for counts
def calculate_stats(df, grade_col, section_col, gpa_map):
    ...
    overall_dist_count = df_copy[grade_col].value_counts(normalize=False).sort_index() # ADDED
    ...
    section_dist_count = pd.DataFrame() # ADDED
    try:
        section_dist_percent = df_copy.groupby(section_col)[grade_col].value_counts(normalize=True).unstack(fill_value=0)
        section_dist_count = df_copy.groupby(section_col)[grade_col].value_counts(normalize=False).unstack(fill_value=0) # ADDED
...
# FIX 2: Use a more diverse palette
SECTION_COLOR_PALETTE = [cm.get_cmap('tab20')(i) for i in range(20)] # Provides 20 distinct RGB(A) colors
...
def get_section_color_fixed(section_name):
    ...
    if str_section not in section_color_map_generated:
        # Convert RGBA from Matplotlib to hex for CSS
...
# FIX 6: Refined highlight_upgraded function
def highlight_upgraded(row, upgraded_students_set, id_col='StudentID', first_col_name='FirstName', last_col_name='LastName'):
    ...
    default_style = '' 
    output_styles = pd.Series(default_style, index=row.index) # Styles for the full row initially
    
    current_row_student_id = row.get(id_col)

    if current_row_student_id and str(current_row_student_id) in upgraded_students_set:
        # Determine which cells (within the subset this function will be applied to) should get the style
        # The 'subset' parameter of styler.apply() will filter which of these output_styles are used.
        if first_col_name in row.index: # Check if FirstName column exists in the row
            output_styles[first_col_name] = highlight_style
        if last_col_name in row.index: # Check if LastName column exists
            output_styles[last_col_name] = highlight_style
        
        # If StudentID itself is to be highlighted (e.g., no name columns, or if we always want ID highlighted)
        # This logic highlights ID if name columns were not available or not highlighted.
        name_cols_actually_highlighted = False
        if first_col_name in row.index and output_styles[first_col_name] == highlight_style:
            name_cols_actually_highlighted = True
        if last_col_name in row.index and output_styles[last_col_name] == highlight_style:
            name_cols_actually_highlighted = True

        if id_col in row.index and not name_cols_actually_highlighted:
            output_styles[id_col] = highlight_style
...
st.title("Iterative Grading Assistant v1.12") # Version update
st.info("**Workflow:**\n"
...
# FIX 1: For persistent column mappings
if 'column_mappings' not in st.session_state:
    st.session_state.column_mappings = {'col_first': None, 'col_last': None, 'col_id': None, 'col_score': None, 'col_section': None, 'file_id': None}
...
# FIX 8: For "points near cutoff" override
...
# FIX 8: Sidebar input for "points_near_cutoff" initializes/updates active session state
...
st.sidebar.number_input(
    ...
    value=st.session_state.points_near_cutoff_active, # Read from active state
    step=0.1, format="%.1f",
    key='points_near_num_v5_sidebar_key', # Unique key for this widget
    on_change=update_active_points_from_sidebar, # Update active state when this changes
...
with col_cutoff_table:
    ...
    if st.session_state.active_cutoffs:
        ...
        # FIX 7: Copy Cutoffs Feature
        st.subheader("Copyable Active Cutoffs")
        cutoff_str_list = []
        for grade, score in st.session_state.active_cutoffs.items():
            cutoff_str_list.append(f"    '{grade}': {score:.2f}") # Indent for readability
        cutoff_display_str = "active_cutoffs = {\n" + ",\n".join(cutoff_str_list) + "\n}"
        st.code(cutoff_display_str, language='python')
        st.caption("Manually copy the text above.")
    else:
...
with col_width_table:
    if st.session_state.active_cutoffs:
        ...
        for i, grade_key in enumerate(grades_in_desc_order):
            ...
            widths[grade_label] = f"{width_val:.2f} [{start_score:.2f} – <{upper_bound:.2f})" # Using en-dash and <
        if widths:
...
# Callback to reset dependent states when a new file is uploaded
def new_file_uploaded_callback():
    # Reset column mappings for the new file
    st.session_state.column_mappings = {'col_first': None, 'col_last': None, 'col_id': None, 'col_score': None, 'col_section': None, 'file_id': None}
    # Reset other data-dependent states
    st.session_state.data_loaded = False
    st.session_state.processed_df = None
    st.session_state.df_graded = None
    st.session_state.stats_results = None
    st.session_state.upgraded_students = set()
    # It's often good to also reset manual_override_values for cutoffs, or let user decide
    # st.session_state.active_cutoffs = None # Or back to initial if preferred
    # st.session_state.manual_override_values = {}


uploaded_file = st.file_uploader("Upload course scores (CSV or Excel)", type=["csv", "xlsx"], key="file_uploader_v13_main", on_change=new_file_uploaded_callback)

if uploaded_file:
    # Check if this is a "new" upload vs a rerun with the same file
    # Using file_id (a unique identifier for the uploaded file instance)
    # This part might be tricky if file_id is not stable or always available as expected.
    # A simpler check is if st.session_state.column_mappings['file_id'] != uploaded_file.file_id
    # For now, we rely on the on_change callback.
    ...
    try:
        ...
        # FIX 1: Use session state for column mapping persistence
        ...
        # Use new keys for selectboxes after major change to ensure state reset if needed
        key_suffix = "_fix1_v13" 
        ...
        if col_score_selection != "<Select Column>" and col_section_selection != "<Select Column>":
            ...
            st.session_state.column_mappings['file_id'] = uploaded_file.id # Store ID of current file for persistence logic
            ...
            if df.empty:
                ...
                st.session_state.df_graded = None; st.session_state.stats_results = None; # Reset downstream
        else:
...
if st.session_state.data_loaded and df_display is not None and st.session_state.active_cutoffs is not None:
    ...
    col_defs = [st.columns(4) for _ in range(len(grade_keys_in_order) // 4 + (1 if len(grade_keys_in_order) % 4 > 0 else 0) )]
    flat_cols = [item for sublist in col_defs for item in sublist]
    ...
    for i, key in enumerate(grade_keys_in_order):
        with flat_cols[i]:
            ...
            manual_cutoffs_input[key] = st.number_input(grade_label, value=default_val, step=0.1, key=f'man_{key}_v14', format="%.2f")
    ...
    # FIX 8: Input for "points_near_cutoff_active" in the main panel
    st.session_state.points_near_cutoff_active = st.number_input(
        ...
        step=0.1, format="%.1f",
        key='points_near_cutoff_override_main_v2',
    ...
    with slider_col: num_bins = st.slider("Histogram Bins", 5, 50, 25, key='hist_bins_v11')
    with hist_col:
    ...
    if 'Section' in df_display.columns:
        ...
        fig_strip, ax_strip = plt.subplots(figsize=(10, max(4, len(unique_sections_plot) * 0.6))) # Increased height factor
        sns.stripplot(data=df_display, x='Score', y='Section', hue='Section', order=unique_sections_plot, hue_order=unique_sections_plot, 
                      ax=ax_strip, jitter=0.3, size=5, alpha=0.8, legend="auto", palette=plot_palette_for_strip) # Increased size/alpha
        ...
        if len(unique_sections_plot) > 1 and len(unique_sections_plot) <= 20 : # Limit legend for too many sections
    ...
    if df_for_near_cutoff_check is not None and 'Letter_Grade' in df_for_near_cutoff_check.columns and \
       not df_for_near_cutoff_check['Letter_Grade'].astype(str).str.contains('Error', na=False).any():
        ...
        for boundary_score_val in unique_cutoff_scores_for_plot: 
            min_score_for_nearby = boundary_score_val - active_points_near_cutoff_for_display # Use overridden value
    ...
    if not students_near_df_local.empty:
        ...
        if student_identifier_col_name:
            ...
            selected_display_labels = st.multiselect("Highlight Students Below Cutoff as Upgraded:", options=student_options_for_multiselect, default=currently_selected_student_labels, key="manual_upgrade_select_v6")
    ...
    if st.session_state.stats_results and st.session_state.stats_results.get("error") is None:
        ...
        # FIX 3: Display counts and percentages
        if "overall_dist_percent" in results and not results['overall_dist_percent'].empty:
            try:
                ...
                if combined_display_list:
                    ...
                    st.markdown(combined_dist_display_df.to_html(escape=False, index=True), unsafe_allow_html=True) # Sticking to HTML for complex string format
        ...
        with col_gpa_plot: # FIX 4A: GPA Boxplot check
        ...
        if st.session_state.df_graded is not None:
            ...
            # FIX 6: Apply refined highlight_upgraded
            ...
            # FIX 5: Use st.dataframe for interactive table
            ...
            selected_section_download = st.selectbox("Select section to download:", options=sections_for_download, key="download_section_select_v11")
            ...
            try:
                ...
                if csv_data_download:
                    ...
                    st.download_button(label=f"Download Grades for {selected_section_download}", data=csv_data_download, file_name=file_name_dl, mime='text/csv', key=f"download_btn_{selected_section_download}_v11")
...
st.sidebar.markdown("---"); st.sidebar.info("Iterative Grading Tool v1.12")
