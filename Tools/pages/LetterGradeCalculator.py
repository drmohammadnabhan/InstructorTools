...
def apply_row_styles(row):
    ...
    if grade_idx != -1: styles[grade_idx] = f'background-color: {GRADE_COLORS.get(str(row.iloc[grade_idx]), GRADE_COLORS["default"])}'
...
st.title("Iterative Grading Assistant v1.6") # Version update
st.info("**Workflow:**\n"
...
uploaded_file = st.file_uploader("Upload course scores (CSV or Excel)", type=["csv", "xlsx"], key="file_uploader_v7") # Incremented key
...
if uploaded_file:
    try:
        ...
        cols = df_upload.columns.tolist(); cols_with_none = ["<Select Column>"] + cols
        col_first = st.selectbox("Select First Name Column (Optional)", options=cols_with_none, index=0, key='sel_first_v7')
        col_last = st.selectbox("Select Last Name Column (Optional)", options=cols_with_none, index=0, key='sel_last_v7')
        col_id = st.selectbox("Select Student ID Column (Optional)", options=cols_with_none, index=0, key='sel_id_orig_v8')
        col_score = st.selectbox("Select Score Column*", options=cols_with_none, index=0, key='sel_score_orig_v8')
        col_section = st.selectbox("Select Section Column*", options=cols_with_none, index=0, key='sel_section_orig_v8')
        ...
        if col_score != "<Select Column>" and col_section != "<Select Column>":
             ...
             initial_rows = len(df); df.dropna(subset=['Score'], inplace=True); removed_rows = initial_rows - len(df)
...
if st.session_state.data_loaded and df_display is not None and st.session_state.active_cutoffs is not None:
    ...
    for i, key in enumerate(grade_keys_in_order):
        with cols_map[i]:
             ...
             manual_cutoffs_input[key] = st.number_input(grade_label, value=float(default_val), step=0.1, key=f'man_{key}_v8', format="%.2f")
    ...
    except Exception as e: st.error(f"An error occurred during grade/stats calculation: {e}"); st.session_state.stats_results = None; st.session_state.df_graded = None
    ...
    with slider_col: num_bins = st.slider("Histogram Bins", min_value=5, max_value=50, value=25, key='hist_bins_v5')
    with hist_col:
    ...
    if 'Section' in df_display.columns:
        ...
        ax_strip.legend(title='Section', bbox_to_anchor=(1.02, 1), loc='upper left'); plt.tight_layout(rect=[0, 0, 0.9, 1]); st.pyplot(fig_strip); plt.close(fig_strip)
    ...
    if st.session_state.stats_results and st.session_state.stats_results.get("error") is None:
        ...
        if "overall_dist" in results and not results['overall_dist'].empty and \
           "section_dist" in results and not results['section_dist'].empty:
            try:
                ...
                all_grades_ordered = ['A+', 'A', 'B+', 'B', 'C+', 'C', 'D+', 'D', 'F', 'Invalid Score'] # Add potential errors/invalid
                all_grades_ordered = [g for g in all_grades_ordered if g in overall_series.index or g in section_dist_df.columns] # Keep only grades present
                ...
                # Apply styling using the corrected method
                ...
                st.markdown(styler_dist.to_html(escape=False, index=True), unsafe_allow_html=True)
                 ...
                 st.write("Raw Overall Distribution:", results.get('overall_dist')) # Fallback
                 st.write("Raw Section Distribution:", results.get('section_dist')) # Fallback
        ...
        # ... (Section GPA Table, ANOVA, Boxplot logic remains the same) ...
        with col_gpa1:
        ...
        if st.session_state.df_graded is not None:
             ...
             selected_section = st.selectbox("Select section to download:", options=sections, key="download_section_select_v5") # Incremented key
             ...
             try:
                ...
                if csv_data:
                    ...
                    st.download_button(label=f"Download Grades for {selected_section}", data=csv_data, file_name=file_name, mime='text/csv', key=f"download_{selected_section}_v5") # Use dynamic key
...
# Footer
st.sidebar.markdown("---")
st.sidebar.info("Iterative Grading Tool v1.6")
