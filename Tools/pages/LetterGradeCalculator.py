# Full code block with fixes for IndentationError and .id AttributeError
...
def calculate_stats(df, grade_col, section_col, gpa_map):
    ...
    except Exception: st.warning("Could not generate section-wise grade distributions.") # Simplified error message
    ...
    if len(section_groups_for_anova) > 1:
        ...
        except Exception as e: anova_result = f"ANOVA Error: {e}" # Simplified error message
...
def highlight_upgraded(row, upgraded_students_set, id_col='StudentID', first_col_name='FirstName', last_col_name='LastName'):
    ...
    if current_row_student_id and str(current_row_student_id) in upgraded_students_set:
        ...
        if last_col_name in row.index and output_styles[last_col_name] == highlight_style: name_cols_actually_highlighted = True # Fixed: was and not name_cols_actually_highlighted
...
st.title("Iterative Grading Assistant v1.14") # Version update
st.info("**Workflow:**\n"
...
def update_active_points_from_sidebar(): st.session_state.points_near_cutoff_active = st.session_state.points_near_num_v6_sidebar_key # Key of sidebar widget
...
uploaded_file = st.file_uploader("Upload scores (CSV/Excel)", ["csv", "xlsx"], key="file_uploader_v15_main_fix", on_change=new_file_uploaded_callback) # Changed key
...
if uploaded_file:
    try:
        ...
        key_sfx = "_fix_v15" # Ensure keys are consistent
        ...
        if col_score_selection != "<Select Column>" and col_section_selection != "<Select Column>":
            ...
            current_map['col_score'] = col_score_selection
            current_map['col_section'] = col_section_selection
            # Removed uploaded_file.id usage

            df = df_upload.copy()
            df.rename(columns={col_score_selection: 'Score', col_section_selection: 'Section'}, inplace=True)
            final_cols = ['Score', 'Section']
            if current_map['col_id']:
                df.rename(columns={current_map['col_id']: 'StudentID'}, inplace=True)
                final_cols.append('StudentID')
            elif 'StudentID' not in df.columns:
                df['StudentID'] = 'Stud_' + df.index.astype(str)
                final_cols.append('StudentID')
            if current_map['col_first']:
                df.rename(columns={current_map['col_first']: 'FirstName'}, inplace=True)
                final_cols.append('FirstName')
            if current_map['col_last']:
                df.rename(columns={current_map['col_last']: 'LastName'}, inplace=True)
                final_cols.append('LastName')

            df = df[[c for c in final_cols if c in df.columns]]
            df['Score'] = pd.to_numeric(df['Score'], errors='coerce')
            initial_rows = len(df) # Store initial rows before dropna
            df.dropna(subset=['Score'], inplace=True)
            removed_rows = initial_rows - len(df) # Calculate removed rows
            if removed_rows > 0: st.warning(f"Removed {removed_rows} rows with invalid/missing scores.")

            if df.empty:
                st.error("No valid score data.")
                st.session_state.data_loaded, st.session_state.processed_df = False, None
            else:
            ...
            st.warning("Select Score & Section columns.")
            st.session_state.data_loaded, st.session_state.processed_df = False, None
...
if st.session_state.data_loaded and df_display is not None and st.session_state.active_cutoffs:
        ...
        for c_idx in range(cols_per_row):
            ...
            if grade_idx < num_grades:
                ...
                with cols[c_idx]:
                    ...
                    manual_cutoffs_input[key] = st.number_input(grade_label, value=default_val, step=0.1, key=f'man_{key}_v16', format="%.2f") # Incremented key
    ...
    st.session_state.points_near_cutoff_active = st.number_input("Range for 'Students Below Cutoffs'", 0.1, 10.0, st.session_state.points_near_cutoff_active, 0.1, "%.1f", key='points_near_override_v5') # Incremented key
    ...
    sns.histplot(df_display['Score'], kde=True, ax=ax_hist, bins='auto', stat="density", color=get_section_color_fixed("OverallDistribution"))
    ...
    if 'Section' in df_display.columns:
        ...
        elif ax_strip.get_legend() is not None: ax_strip.get_legend().remove() # Remove legend if not needed
    ...
    if df_for_near_cutoff_check is not None and 'Letter_Grade' in df_for_near_cutoff_check.columns and \
       not df_for_near_cutoff_check['Letter_Grade'].astype(str).str.contains('Error', na=False).any():
        ...
        for gk_s, sv_s in st.session_state.active_cutoffs.items():
            if gk_s == 'F_Max': continue
            gl_s = gk_s.replace('_Start', '')
        ...
        if students_near_cutoff_list:
            ...
            # ... (display students_near_df_local)
            st.dataframe(students_near_df_local[[c for c in ['FirstName','LastName','StudentID','Score','Section','Letter_Grade','Target_Grade','Target_Boundary','Points_to_Upgrade'] if c in students_near_df_local.columns]].style.format({"Score": "{:.2f}", "Points_to_Upgrade": "{:.2f}", "Target_Boundary": "{:.2f}"}))
    ...
    st.markdown("Select students from 'Students Below Cutoffs' list to highlight their rows in the final table.")
    if not students_near_df_local.empty and 'StudentID' in students_near_df_local.columns:
        eligible_students_df = students_near_df_local.drop_duplicates(subset=['StudentID']).copy()
        eligible_students_df.loc[:, 'DisplayLabel'] = (eligible_students_df.get('FirstName', '') + " " + eligible_students_df.get('LastName', '') + " (" + eligible_students_df['StudentID'].astype(str) + ")").str.strip()
        student_options = sorted(eligible_students_df['DisplayLabel'].tolist())
        id_map = pd.Series(eligible_students_df['StudentID'].astype(str).values, index=eligible_students_df['DisplayLabel']).to_dict()
        current_labels = [lbl for lbl, sid_val in id_map.items() if str(sid_val) in st.session_state.upgraded_students] # Ensure str comparison
        
        selected_labels = st.multiselect("Highlight Students:", student_options, current_labels, key="manual_upgrade_select_v8") # Incremented key
        newly_selected_ids = {str(id_map.get(lbl)) for lbl in selected_labels if id_map.get(lbl) is not None}
    ...
    elif 'StudentID' not in students_near_df_local.columns and not students_near_df_local.empty :
        st.warning("StudentID column missing in 'Students Near Cutoffs' data, cannot select for highlighting.")
    else: st.markdown("_Students potentially needing upgrades will appear here._")
    ...
    if st.session_state.stats_results and not st.session_state.stats_results.get("error"):
        ...
        if "overall_dist_percent" in results and not results['overall_dist_percent'].empty:
            try:
                # ... (Grade distribution display logic - assuming v1.13 was okay, with counts)
                # This part needs careful construction to avoid errors if some data is missing
                overall_pct = results['overall_dist_percent']; overall_cnt = results['overall_dist_count']
                section_pct = results['section_dist_percent']; section_cnt = results['section_dist_count']
        ...
        col_gpa_table_final, col_gpa_plot_final = st.columns(2) # Use new var names to avoid conflict
        with col_gpa_table_final:
            st.write("**Per Section Avg GPA:**"); # ... (display results['section_stats'])
            ...
            if anova_p is not None:
                if anova_p < alpha: st.markdown(f"<span style='color:orange;'>Significant difference in section GPAs detected (p={anova_p:.3f}).</span>", unsafe_allow_html=True)
        ...
        with col_gpa_plot_final:
            if st.session_state.df_graded is not None and 'GPA' in st.session_state.df_graded.columns and st.session_state.df_graded['GPA'].notna().any():
                try:
                    ...
                    ax_box.set_title("GPA by Section"); plt.xticks(rotation=45, ha='right'); plt.tight_layout(); st.pyplot(fig_box); plt.close(fig_box)

                except Exception as e: st.warning(f"Could not generate GPA boxplot: {e}")
            else: st.warning("GPA data (e.g., all NaN) not available for boxplot. Check grades & GPA scale.")

        st.subheader("Failing Students Analysis") # ... (Failing students display logic - assuming okay)
        ...
        if st.session_state.df_graded is not None:
            ...
            reset_section_color_cycle(); # ... pre-populate section colors ...
            ...
            if 'StudentID' in df_to_style_final.columns and st.session_state.upgraded_students:
                id_c, fn_c, ln_c = 'StudentID', 'FirstName', 'LastName'
                subset_eff = [c for c in [fn_c, ln_c] if c in df_to_style_final.columns] or ([id_c] if id_c in df_to_style_final.columns else [])
                if subset_eff:
                    styler_final = styler_final.apply(highlight_upgraded, axis=1, upgraded_students_set=st.session_state.upgraded_students,
                                                  id_col=id_c, first_col_name=fn_c, last_col_name=ln_c, subset=subset_eff)
            st.dataframe(styler_final.format({"Score": "{:.2f}", "GPA": "{:.2f}"}), use_container_width=True)
            # ... (Download logic - assuming okay) ...
