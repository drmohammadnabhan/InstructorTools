# Full code block incorporating styling refinement (Point 5)
# and previous fixes (rerun, near cutoff input, separate names etc.)
...
def assign_letter_grades_from_starts(scores, start_cutoffs):
    ...
    if not all(boundary_scores[i] < boundary_scores[i+1] for i in range(len(boundary_scores)-1)):
        ...
        if unique_scores_count < total_scores_count:
             if not all(boundary_scores[i] <= boundary_scores[i+1] for i in range(len(boundary_scores)-1)):
                 st.error("Grade boundary scores must increase monotonically (or be equal). Check manual inputs."); return pd.Series(['Error - Non-monotonic Cutoffs'] * len(scores), index=scores.index, dtype='object')
             else: st.warning("Duplicate start scores detected; using unique boundaries.")
        else: st.error("Grade boundaries must increase monotonically. Check manual inputs."); return pd.Series(['Error - Non-monotonic Cutoffs'] * len(scores), index=scores.index, dtype='object')
...
def calculate_stats(df, grade_col, section_col, gpa_map):
    ...
    df['GPA'] = df[grade_col].map(gpa_map); df['GPA'] = pd.to_numeric(df['GPA'], errors='coerce')
    if df['GPA'].isnull().all() and df[grade_col].notna().any() and not df[grade_col].isin(['Invalid Score']).all(): st.warning("GPA calculation resulted in all non-numeric values.")
    overall_dist = df[grade_col].value_counts(normalize=True).sort_index(); overall_gpa = df['GPA'].mean()
    df[section_col] = df[section_col].astype(str)
    section_gpa_means = df.groupby(section_col)['GPA'].mean(); section_stats = df.groupby(section_col).agg(Count=('GPA', 'size'), Valid_GPA_Count = ('GPA', 'count')).reset_index()
...
def get_section_color_fixed(section_name):
    str_section = str(section_name) # Ensure string key
    if str_section not in section_color_map_generated: section_color_map_generated[str_section] = next(color_cycle)
    return section_color_map_generated[str_section]

# Highlight upgraded students (applied row-wise AFTER other styles)
def highlight_upgraded(row, upgraded_students_set, id_col='StudentID', first_col='FirstName', last_col='LastName'):
    highlight = 'background-color: #90EE90 !important;' # LightGreen, !important might help override other styles
    default = ''
    student_id = row.get(id_col)
    style = pd.Series(default, index=row.index) # Start with default styles
    if student_id and str(student_id) in upgraded_students_set: # Check if ID is in the set (convert to str)
        # Apply highlight to available name/id columns
        if first_col in row.index: style[first_col] = highlight
        if last_col in row.index: style[last_col] = highlight
        # Only highlight ID if no name columns exist or were specified
...
st.title("Iterative Grading Assistant v1.10") # Version update
...
a_plus_start_score = st.sidebar.number_input("A+ Start Score (>= score)", 95.0, step=0.1, format="%.2f", help="Min score for A+.")
uniform_grade_gap = st.sidebar.number_input("Uniform Grade Gap (points)", 5.0, step=0.1, min_value=0.1, format="%.2f", help="Initial gap between grade starts.")
points_near_cutoff = st.sidebar.number_input(
    "Show Students Below Cutoff (within X points)", 0.1, 10.0, 1.5, 0.1, format="%.1f", key='points_near_num_v4',
...
with col_cutoff_table:
     ...
     else: cutoff_display_area.warning("Calculate initial cutoffs first.")
with col_width_table: # Display Grade Widths
    if st.session_state.active_cutoffs:
        ...
        for i, grade in enumerate(sorted_grades):
            ...
            width = upper_bound - start_score; width = 0.0 if np.isclose(width, 0) else width
...
uploaded_file = st.file_uploader("Upload course scores (CSV or Excel)", type=["csv", "xlsx"], key="file_uploader_v11") # Incremented key
...
if uploaded_file:
    try:
        ...
        col_first = st.selectbox("First Name Column (Optional)", options=cols_with_none, index=0, key='sel_first_v11')
        col_last = st.selectbox("Last Name Column (Optional)", options=cols_with_none, index=0, key='sel_last_v11')
        col_id = st.selectbox("Student ID Column (Optional)", options=cols_with_none, index=0, key='sel_id_orig_v12')
        col_score = st.selectbox("Score Column*", options=cols_with_none, index=0, key='sel_score_orig_v12')
        col_section = st.selectbox("Section Column*", options=cols_with_none, index=0, key='sel_section_orig_v12')
...
if st.session_state.data_loaded and df_display is not None and st.session_state.active_cutoffs is not None:
    ...
    for i, key in enumerate(grade_keys_in_order):
        with cols_map[i]:
             ...
             manual_cutoffs_input[key] = st.number_input(grade_label, value=float(default_val), step=0.1, key=f'man_{key}_v12', format="%.2f")
    ...
    if st.button("Apply Manual Cutoffs & Recalculate"):
        ...
        if not all(scores_list[i] >= scores_list[i+1] for i in range(len(scores_list)-1)): st.error("Manual Start Scores must be >=.")
        else:
    ...
    with slider_col: num_bins = st.slider("Histogram Bins", 5, 50, 25, key='hist_bins_v9')
    with hist_col:
    ...
    df_temp_graded_near = st.session_state.df_graded if st.session_state.df_graded is not None else None # Use final graded df
    if df_temp_graded_near is not None and 'Letter_Grade' in df_temp_graded_near.columns and not df_temp_graded_near['Letter_Grade'].astype(str).str.contains('Error', na=False).any():
        ...
        for boundary_score in active_cutoff_scores_asc:
            ...
            nearby_df = df_temp_graded_near[(df_temp_graded_near['Score'] >= min_score_below) & (df_temp_graded_near['Score'] < boundary_score)].copy()
        ...
        if students_near_cutoff_list:
            students_near_df = pd.concat(students_near_cutoff_list).sort_values(['Target_Boundary', 'Score']) # Keep local var
            name_cols_near = [];
            ...
            st.caption("Tip: To potentially upgrade a student, note their 'Target_Boundary' score. Adjust the 'Start Score' for the 'Target_Grade' in the section above, then click 'Apply'. Optionally, mark the student in the 'Manual Upgrades' section below for highlighting.")
    ...
    # --- Manual Upgrade Selection (using only near cutoff students) ---
    st.header("Manual Upgrades (Highlighting Only)")
    st.markdown("Select students *from the 'Students Below Cutoffs' list* whose rows should be highlighted green in the final table.")
    # <<< Modified Selection Logic >>>
    if not students_near_df.empty: # Use the df generated above
        student_identifier_col = 'StudentID' if 'StudentID' in students_near_df.columns else None
        if student_identifier_col:
            eligible_students_df = students_near_df.drop_duplicates(subset=[student_identifier_col])
            ...
            else: eligible_students_df['DisplayLabel'] = eligible_students_df[student_identifier_col]
            ...
            selected_labels = st.multiselect("Highlight Students Below Cutoff as Upgraded:", options=student_options, default=currently_selected_labels, key="manual_upgrade_select_v4")
            newly_selected_ids = set(student_id_map.get(label) for label in selected_labels if student_id_map.get(label) is not None)
            # Use button to confirm selection change to avoid excessive reruns from multiselect itself
    ...
    else: st.markdown("_Students potentially needing upgrades will appear here once grades are calculated._")
    ...
    if st.session_state.stats_results and st.session_state.stats_results.get("error") is None:
        results = st.session_state.stats_results; st.subheader("Grade Distributions")
        if "overall_dist" in results and not results['overall_dist'].empty and "section_dist" in results and not results['section_dist'].empty:
            try:
                overall_series = results['overall_dist'].rename('Overall (%)') * 100; section_dist_df = results['section_dist'] * 100
                ...
                section_dist_df = section_dist_df.reindex(columns=present_grades, fill_value=0); overall_series = overall_series.reindex(present_grades, fill_value=0)
                combined_dist = pd.concat([overall_series, section_dist_df.T], axis=1).fillna(0); combined_dist.index.name = 'Grade'
        ...
        overall_gpa_val = results.get('overall_gpa', np.nan); st.write(f"**Overall Avg GPA:** {overall_gpa_val:.2f}" if pd.notna(overall_gpa_val) else "**Overall Avg GPA:** N/A"); st.write("---")

        st.subheader("Section GPA Comparison"); col_gpa1, col_gpa2 = st.columns(2)
        with col_gpa1:
             st.write("**Per Section Avg GPA:**")
             if "section_stats" in results and not results['section_stats'].empty:
                   st.dataframe(results['section_stats'][['Section', 'Avg_GPA', 'Count']].style.format({"Avg_GPA": "{:.2f}"}, na_rep='N/A', precision=2))
             else: st.write("N/A")
             st.write(f"**ANOVA Result:** {results.get('anova_result', 'N/A')}"); anova_p = results.get('anova_p_value');
        ...
        with col_gpa2: # Boxplot unchanged
             try:
                 if st.session_state.df_graded is not None and 'GPA' in st.session_state.df_graded.columns and st.session_state.df_graded['GPA'].notna().any():
                     ...
                     sns.boxplot(data=st.session_state.df_graded, x='Section', y='GPA', ax=ax_box, order=sorted_sections); ax_box.set_title("GPA Distribution by Section"); plt.xticks(rotation=45, ha='right'); st.pyplot(fig_box); plt.close(fig_box)
        ...
        if st.session_state.df_graded is not None and 'Letter_Grade' in st.session_state.df_graded.columns:
            ...
            if passing_score is not None:
                ...
                if not failing_students.empty:
                    failing_students['Points_Below_Pass'] = passing_score - failing_students['Score']; failing_students.sort_values('Points_Below_Pass', ascending=True, inplace=True)
                    st.write(f"Passing Score (D Start): {passing_score:.2f}"); fail_cols = ['Score', 'Section', 'Points_Below_Pass'];
                    ...
                    cols_fail_exist = [col for col in fail_cols if col in failing_students.columns]; st.dataframe(failing_students[cols_fail_exist].style.format({"Score": "{:.2f}", "Points_Below_Pass": "{:.2f}"}))
        ...
        if st.session_state.df_graded is not None:
             ...
             # Apply grade gradient column-wise
             ...
             # Apply section color column-wise
             ...
             if 'StudentID' in df_to_display.columns and st.session_state.upgraded_students:
                 id_col_name = 'StudentID'; name_cols = [col for col in ['FirstName', 'LastName'] if col in df_to_display.columns]
                 highlight_cols = name_cols if name_cols else ([id_col_name] if id_col_name in df_to_display.columns else []) # Highlight name cols if exist, else ID col
                 if highlight_cols:
                      styler = styler.apply(highlight_upgraded, upgraded_students_set=st.session_state.upgraded_students, id_col=id_col_name, axis=1, subset=highlight_cols)
             ...
             selected_section = st.selectbox("Select section to download:", options=sections, key="download_section_select_v9")
             ...
             try:
                ...
                if csv_data:
                    ...
                    st.download_button(label=f"Download Grades for {selected_section}", data=csv_data, file_name=file_name, mime='text/csv', key=f"download_{selected_section}_v9")
...
# Footer
st.sidebar.markdown("---")
st.sidebar.info("Iterative Grading Tool v1.10")
